    ...
    class DTHMain
    {
        ...
        public static void AddUserPL()
        {
            try
            {
                ...
                //Use Regex.IsMatch to chk if the entered value is valid Email
                ...
                //Use Regex.IsMatch to chk if the entered value is valid mobile no
                bool chkname = Regex.IsMatch(MNo, @"[0-9]{10}");
                if (chkname == false)
                {
                    throw new ExceptionDTH("Enter Valid Mobile No");
                }
                // If the regex match is successful, store the applicant name into the Entity object
                else
                {
                ...
                //Use Regex.IsMatch to chk if channel name is valid
                ...
                // If the Parsing is successful, store the ChannelCategory into the Entity object
                else
                {
        ...
        public static void DisplayPL()
        {
            ...
            {
                if (temp.unsubscription == "true")
                {
                    Console.WriteLine("\nSubscription ID :{0}\nEmail :{1}\n Channel Name :{2} \nChannelCategory :{3} \nSubscription Date  :{4}\nPhone NO:{5}\n Unsubscription:{6}\n Unsuscription Date:{7}\n", temp.SubscriptionId, temp.Email, temp.ChannelName, temp.ChannelCategory, temp.subscription_date, temp.MobileNo, temp.unsubscription, temp.unsubscription_date);
                }
                else
                {
